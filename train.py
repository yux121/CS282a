...
def train(args):
    ...
    for epoch in range(int(args.reload_from/n_iters)+1, config['nb_epoch']+1): 
        ...
        for batch in data_loader:
            ...
            if itr_global % n_iters == 0:
                ckpt_path = f'./output/{args.model}/{args.dataset}/models/code_nl_aug/step{itr_global}.h5'
...
def validate(valid_set, model, pool_size, K, sim_measure):
    ...
    for batch in tqdm(data_loader):        
        if len(batch) == 18: # names, name_len, apis, api_len, toks, tok_len, descs, desc_len, bad_descs, bad_desc_len
